...
class SegmentationHead(nn.Module):
    def __init__(
        ...
        assert head_type in ["LinearClassifier", "DPTClassifier"]
        name = f"sseg_{head_type}_k{kernel_size}"
        self.name = name

        if head_type == "LinearClassifier":
            self.head = LinearClassifier(feat_dim, num_labels=num_labels)
        elif head_type == "DPTClassifier":
            self.head = DPTClassifier(feat_dim, hidden_dim, kernel_size, num_labels=num_labels)
        else:
            raise ValueError(f"Unknown head type: {self.head_type}")

    def forward(self, feats, output_size=None):
        return self.head(feats, output_size)
...
class LinearClassifier(torch.nn.Module):
    ...
    def forward(self, embeddings, output_size=None):
        
        output = self.classifier(embeddings)
        
        if output_size is not None:
            output = interpolate(output, size=output_size, mode='bilinear', align_corners=False)

        return output 
    
class DPTClassifier(nn.Module):
    def __init__(self, input_dims, hidden_dim=512, kernel_size=3, num_labels=27):
        super().__init__()
        ...
        self.classifier = LinearClassifier(hidden_dim, num_labels=num_labels)
    ...
    def forward(self, feats, output_size=None):
        ...
        feats = [interpolate(x, scale_factor=2, mode="bilinear") for x in feats]
        ...
        out = interpolate(out, scale_factor=4, mode="bilinear")
        out = self.out_conv(out)
        
        out = self.classifier(out, output_size)
        return out
...
class DepthBinPrediction(nn.Module):
    ...
    def forward(self, prob):
        if self.bins_strategy == "UD":
            ...
            bins = torch.logspace(
                math.log10(self.min_depth), math.log10(self.max_depth),
                self.n_bins, device=prob.device,
            )
...
class DPT(nn.Module):
    ...
    def forward(self, feats):
        ...
        out = interpolate(out, scale_factor=4, mode="bilinear")
        out = self.out_conv(out)
        out = interpolate(out, scale_factor=2, mode="bilinear")
        return out
...
class Linear(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size=1):
        super().__init__()
        if not isinstance(input_dim, (int, np.integer)):
