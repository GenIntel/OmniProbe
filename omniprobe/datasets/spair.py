...
class SPairDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        Inputs:
            root: Path to the SPair-71k dataset root directory
    ...
    def __getitem__(self, index, square=True):
        ...
        seg_i = img_i #self.get_mask(class_name, view_i, bbox=bbx_i, square=square)
        img_j = self.get_image(class_name, view_j, bbx_j, square=square)
        seg_j = img_j #self.get_mask(class_name, view_j, bbox=bbx_j, square=square)
    ...
    def get_mask(self, class_name, image_name, bbox=None, square=False):
        rel_path = f"Re_segmentation/{class_name}/{image_name}.png"
        path = os.path.join(self.root, rel_path)
    ...
    def get_pair_annotations(self):
        ...
        annots = []
        for _path in annot_files:
            with open(_path) as f:
                annots.append(json.load(f))
        return annots
