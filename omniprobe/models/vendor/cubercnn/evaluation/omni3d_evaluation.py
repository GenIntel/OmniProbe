...
import logging
from omniprobe.models.vendor.cubercnn.data import Omni3D
from pytorch3d import _C
import torch.nn.functional as F

from pytorch3d.ops.iou_box3d import _box_planes, _box_triangles

import omniprobe.models.vendor.cubercnn.vis.logperf as utils_logperf
...
class Omni3DEvaluationHelper:
    ...
    #     try:
    #         log_str_2D = results.get('log_str_2D', '')  # Provide default empty string if None
    #         formatted_log_str = log_str_2D.replace('mode=2D', '{} iter={} mode=2D'.format(dataset_name or 'Unknown', self.iter_label or 'N/A'))
    #         logger.info('\n' + formatted_log_str)
    #     except Exception as e:
    #         logger.error(f"Error formatting log string: {e}")
    ...
    #         try:
    #             log_str_3D = results.get('log_str_3D', '')  # Provide default empty string if None
    #             formatted_log_str = log_str_3D.replace('mode=3D', '{} iter={} mode=3D'.format(dataset_name or 'Unknown', self.iter_label or 'N/A'))
    #             logger.info('\n' + formatted_log_str)
    #         except Exception as e:
    #             logger.error(f"Error formatting log string: {e}")
    ...
    def evaluate(self, dataset_name):
        ...
        Args:
            dataset_name (str): The dataset split name to be evaluated.
        """

        try:
            if dataset_name not in self.results:
                # Run evaluation and cache the results
                self.results[dataset_name] = self.evaluators[dataset_name].evaluate()

            results = self.results[dataset_name]

            # Safely log 2D results
            try:
                log_str_2D = results.get('log_str_2D', '')  # Provide default empty string if missing
                formatted_log_str = log_str_2D.replace(
                    'mode=2D', '{} iter={} mode=2D'.format(dataset_name or 'Unknown', self.iter_label or 'N/A')
                )
                logger.info('\n' + formatted_log_str)
            except Exception as e:
                logger.error(f"Error formatting 2D log string for {dataset_name}: {e}")

            # Store evaluations per category per area (2D)
            for key, item in results.get('bbox_2D_evals_per_cat_area', {}).items():
                if key not in self.evals_per_cat_area2D:
                    self.evals_per_cat_area2D[key] = []
                self.evals_per_cat_area2D[key] += item

            if not self.only_2d:
                # Store evaluations per category per area (3D)
                for key, item in results.get('bbox_3D_evals_per_cat_area', {}).items():
                    if key not in self.evals_per_cat_area3D:
                        self.evals_per_cat_area3D[key] = []
                    self.evals_per_cat_area3D[key] += item

                # Safely log 3D results
                try:
                    log_str_3D = results.get('log_str_3D', '')
                    formatted_log_str = log_str_3D.replace(
                        'mode=3D', '{} iter={} mode=3D'.format(dataset_name or 'Unknown', self.iter_label or 'N/A')
                    )
                    logger.info('\n' + formatted_log_str)
                except Exception as e:
                    logger.error(f"Error formatting 3D log string for {dataset_name}: {e}")

            # Full model category names
            category_names = self.filter_settings.get('category_names', [])

            # The set of categories present in the dataset; ensure no duplicates
            categories = {cat for cat in category_names if f'AP-{cat}' in results.get('bbox_2D', {})}
            assert len(categories) == len(set(categories)), "Duplicate categories detected!"

            # Default values (set to NaN)
            general_2D, general_3D, omni_2D, omni_3D = (np.nan,) * 4

            # Compute 2D and 3D performance for categories in dataset
            general_2D = np.mean([results['bbox_2D'].get(f'AP-{cat}', np.nan) for cat in categories])
            if not self.only_2d:
                general_3D = np.mean([results['bbox_3D'].get(f'AP-{cat}', np.nan) for cat in categories])

            # Compute 2D and 3D performance on Omni3D categories
            omni3d_dataset_categories = get_omni3d_categories(dataset_name)
            if omni3d_dataset_categories.issubset(categories):
                omni_2D = np.mean([results['bbox_2D'].get(f'AP-{cat}', np.nan) for cat in omni3d_dataset_categories])
                if not self.only_2d:
                    omni_3D = np.mean([results['bbox_3D'].get(f'AP-{cat}', np.nan) for cat in omni3d_dataset_categories])

            self.results_omni3d[dataset_name] = {"iters": self.iter_label, "AP2D": omni_2D, "AP3D": omni_3D}

            # Performance analysis
            extras_AP15 = results.get('bbox_3D', {}).get('AP15', np.nan)
            extras_AP25 = results.get('bbox_3D', {}).get('AP25', np.nan)
            extras_AP50 = results.get('bbox_3D', {}).get('AP50', np.nan)
            extras_AP70 = results.get('bbox_3D', {}).get('AP70', np.nan)
            extras_APn = results.get('bbox_3D', {}).get('APn', np.nan)
            extras_APm = results.get('bbox_3D', {}).get('APm', np.nan)
            extras_APf = results.get('bbox_3D', {}).get('APf', np.nan)

            self.results_analysis[dataset_name] = {
                "iters": self.iter_label,
                "AP2D": general_2D, "AP3D": general_3D,
                "AP3D@15": extras_AP15, "AP3D@25": extras_AP25, "AP3D@50": extras_AP50, "AP3D@70": extras_AP70,
                "AP3D-N": extras_APn, "AP3D-M": extras_APm, "AP3D-F": extras_APf
            }
            ...
            results_cat = OrderedDict()
            for cat in category_names:
                cat_2D = results.get('bbox_2D', {}).get(f'AP-{cat}', np.nan)
                cat_3D = results.get('bbox_3D', {}).get(f'AP-{cat}', np.nan) if not self.only_2d else np.nan
            ...
            logger.error(f"Error during evaluation of dataset {dataset_name}: {e}", exc_info=True)
    ...
    def summarize_all(self,):
        ...
        metrics = ["AP", "AP15", "AP25", "AP50", "AP70", "APn", "APm", "APf"]
        ...
        extras_AP15, extras_AP25, extras_AP50, extras_AP70, extras_APn, extras_APm, extras_APf = (np.nan,) * 7
        if not self.only_2d:
            extras_AP15 = results3D['AP15']
            extras_AP25 = results3D['AP25']
            extras_AP50 = results3D['AP50']
            extras_AP70 = results3D['AP70']
...
class Omni3DEvaluator(COCOEvaluator):
    ...
    def _derive_omni_results(self, omni_eval, iou_type, mode, class_names=None):
        ...
        metrics = {
            ...
            "3D": ["AP", "AP15", "AP25", "AP50", "AP70", "APn", "APm", "APf"],
        }[mode]
    ...
    def _eval_predictions(self, predictions, img_ids=None):
        ...
        for task in sorted(tasks):
            ...
            try:
                result = _evaluate_predictions_on_omni(
                    self._omni_api,
                    omni_results,
                    task,
                    img_ids=img_ids,
                ...
                ) if len(omni_results) > 0 else None  # cocoapi does not handle empty results well

                if result is None:
                    print(f"Warning: Evaluation returned None for task {task}. Skipping...")
                    continue  # Skip this iteration if no valid result

                evals, log_strs = result
                ...
                self._results["log_str_2D"] = log_strs.get("2D", "No 2D evaluation available")

                if "3D" in log_strs:
                    self._results["log_str_3D"] = log_strs["3D"]

            except TypeError as e:
                print(f"Error: Failed to unpack evaluation results for task {task}.")
                print(f"Exception: {e}")
                print(f"Skipping task {task} due to invalid return from _evaluate_predictions_on_omni().")
                continue  # Skip the problematic task but allow others to run
...
class Omni3Deval(COCOeval):
    ...
    def accumulate(self, p = None):
        ...
        for k, (k0, catId) in enumerate(zip(k_list, catid_list)):
            ...
            for a, a0 in enumerate(a_list):
                ...
                if has_precomputed_evals:
                    # a category can be absent entirely, e.g. when a weak
                    # model produced no predictions for a dataset
    ...
    def summarize(self):
        ...
        def _summarizeDets(mode):
            ...
            # Define thresholds based on mode (2D or 3D)
            thres = [0.5, 0.75, 0.95] if mode == "2D" else [0.15, 0.25, 0.50, 0.70]

            # Dynamically allocate stats array based on number of thresholds
            stats = np.zeros((10 + len(thres),))
            stats[0], log_str = _summarize(mode, 1)

            # Compute AP for each IoU threshold in `thres`
            for i in range(len(thres)):
                stats[i + 1], log_str = _summarize(
                    mode, 1, iouThr=thres[i], maxDets=params.maxDets[2], log_str=log_str
                )

            # Handling area-based summarization (3 entries)
            offset = len(thres) + 1
            stats[offset], log_str = _summarize(
                mode, 1, areaRng=params.areaRngLbl[1], maxDets=params.maxDets[2], log_str=log_str
            )

            stats[offset + 1], log_str = _summarize(
                mode, 1, areaRng=params.areaRngLbl[2], maxDets=params.maxDets[2], log_str=log_str
            )

            stats[offset + 2], log_str = _summarize(
                mode, 1, areaRng=params.areaRngLbl[3], maxDets=params.maxDets[2], log_str=log_str
            )

            # Handling maxDets summarization (3 entries)
            offset += 3
            stats[offset], log_str = _summarize(
            ...
            # Handling area-based summarization for maxDets (3 entries)
            offset += 3
            stats[offset], log_str = _summarize(
                mode, 0, areaRng=params.areaRngLbl[1], maxDets=params.maxDets[2], log_str=log_str
            )

            stats[offset + 1], log_str = _summarize(
                mode, 0, areaRng=params.areaRngLbl[2], maxDets=params.maxDets[2], log_str=log_str
            )

            stats[offset + 2], log_str = _summarize(
                mode, 0, areaRng=params.areaRngLbl[3], maxDets=params.maxDets[2], log_str=log_str
            )
