...
VERSION = '0.1'
DATASET_PATH = os.getenv("OMNI3D_PATH", "datasets")  # Default to 'datasets' if the variable is not set
...
def get_global_dataset_stats(path_to_stats=None, reset=False):
    ...
    if path_to_stats is None:
        path_to_stats = os.path.join(DATASET_PATH, 'Omni3D', 'stats.json')
...
def simple_register(dataset_name, filter_settings, filter_empty=False, datasets_root_path=None):
    ...
    if datasets_root_path is None:
        datasets_root_path = DATASET_PATH
    
    path_to_json = os.path.join(datasets_root_path, 'Omni3D', dataset_name + '.json')
    path_to_image_root = datasets_root_path
...
def register_and_store_model_metadata(datasets, output_dir, filter_settings=None, datasets_root_path=None):
    ...
    if metadata is not None:
        ...
        omni3d_stats = util.load_json(os.path.join(datasets_root_path, 'Omni3D', 'stats.json'))
