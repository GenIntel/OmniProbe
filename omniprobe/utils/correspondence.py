...
def _get_faiss_module():
    # Import FAISS lazily because many correspondence tasks never use the kNN path,
    # and eager GPU resource initialization can crash otherwise unrelated evaluations.
...
def _get_faiss_resources():
    return _get_faiss_module().StandardGpuResources()
...
def softmax_with_temperature(x: torch.Tensor, beta: float, dim: int = -1) -> torch.Tensor:
    """Stable softmax with temperature (beta)."""
    x = x - x.max(dim=dim, keepdim=True).values
    return torch.softmax(x / beta, dim=dim)


def soft_argmax_2d(heatmaps: torch.Tensor, beta: float = 0.02, window: int = 0) -> torch.Tensor:
    """
    Soft-argmax over 2D heatmaps.

    Args:
        heatmaps: (..., H, W)
        beta: temperature for softmax (lower -> sharper)
        window: if > 0, restrict softmax to a (2*window+1)^2 window around argmax;
                if < 0, apply a gaussian kernel with sigma = -window around argmax.
    Returns:
        (..., 2) with x,y coordinates in heatmap index space.
    """
    *leading, h, w = heatmaps.shape
    ...
    if window != 0:
        ...
        if window > 0:
            # Gather only the local window values to avoid materializing full masks.
            offsets = torch.arange(-window, window + 1, device=hm.device)
            dy, dx = torch.meshgrid(offsets, offsets, indexing="ij")
            dy = dy.reshape(1, -1)
            dx = dx.reshape(1, -1)

            win_y = center_y[:, None] + dy
            win_x = center_x[:, None] + dx
            valid = (win_y >= 0) & (win_y < h) & (win_x >= 0) & (win_x < w)

            win_y = win_y.clamp(0, h - 1)
            win_x = win_x.clamp(0, w - 1)

            batch = torch.arange(n, device=hm.device)[:, None]
            win_vals = hm[batch, win_y, win_x]
            win_vals = win_vals.masked_fill(~valid, -1e9)

            probs = softmax_with_temperature(win_vals, beta=beta, dim=-1)
            win_x_f = win_x.to(probs.dtype)
            win_y_f = win_y.to(probs.dtype)
            exp_x = (probs * win_x_f).sum(dim=-1)
            exp_y = (probs * win_y_f).sum(dim=-1)

            coords = torch.stack((exp_x, exp_y), dim=-1)
            return coords.view(*leading, 2)
        else:
            sigma = float(-window)
            yy = torch.arange(h, device=hm.device)  # (h,)
            xx = torch.arange(w, device=hm.device)  # (w,)

            # Compute 1D Gaussian components separately to save memory
            # gauss_y: (n, h), gauss_x: (n, w)
            gauss_y = torch.exp(-((yy.view(1, h) - center_y.view(n, 1)) ** 2) / (2 * sigma ** 2))
            gauss_x = torch.exp(-((xx.view(1, w) - center_x.view(n, 1)) ** 2) / (2 * sigma ** 2))

            # Apply as separable filter
            hm = hm * gauss_y.unsqueeze(2) * gauss_x.unsqueeze(1)

    probs = softmax_with_temperature(hm.reshape(-1, h * w), beta=beta, dim=-1)
    probs = probs.view(-1, h, w)

    xs = torch.linspace(0, w - 1, w, device=hm.device)
    ys = torch.linspace(0, h - 1, h, device=hm.device)

    prob_x = probs.sum(dim=1)
    prob_y = probs.sum(dim=2)

    exp_x = (prob_x * xs.view(1, w)).sum(dim=1)
    exp_y = (prob_y * ys.view(1, h)).sum(dim=1)
