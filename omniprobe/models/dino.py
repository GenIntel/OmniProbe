...
class DINO(torch.nn.Module):
    def __init__(
        ...
        if "dinov2" in dino_name:
            self.vit = torch.hub.load(
                ...
                f"{dino_name}_{model_name}",
                trust_repo=True,
            )
        else:
            self.vit = torch.hub.load(
                f"facebookresearch/{dino_name}:main",
                f"{dino_name}_{model_name}",
                trust_repo=True,
            )
        self.vit.eval().to(torch.float32)
    ...
    def forward(self, images):
        ...
        for i, x_i in enumerate(embeds):
            cls_tok = x_i[:, 0]
            # Take last num_spatial tokens: works for both register and
            # non-register models since registers sit between CLS and patches.
