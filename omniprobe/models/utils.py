...
class BackboneProtocol(Protocol):
    """Expected interface for backbone model wrappers (documentation only)."""
    checkpoint_name: str
    patch_size: int
    layer: str
    output: str
    feat_dim: Union[int, list[int]]

    def forward(self, images: torch.Tensor) -> Union[torch.Tensor, list[torch.Tensor]]:
        ...


def default_multilayers(num_layers: int) -> list[int]:
    """4 evenly-spaced layer indices for multi-layer feature extraction."""
    return [
        max(num_layers // 4 - 1, 0),
        max(num_layers // 2 - 1, 0),
        max((num_layers * 3) // 4 - 1, 0),
        num_layers - 1,
    ]


def resolve_pretrained_path(*parts: str, prefer_cache: bool = False) -> Path:
    """
    Resolve a checkpoint path while preferring the shared pretrained directory
    or the active local cache over repo-local ad hoc paths.
    """

    explicit_root = os.environ.get("OMNIPROBE_PRETRAINED_MODELS")
    if explicit_root:
        root = Path(explicit_root).expanduser()
    elif prefer_cache and os.environ.get("TORCH_HOME"):
        root = Path(os.environ["TORCH_HOME"]).expanduser() / "checkpoints"
    else:
        root = Path(__file__).resolve().parents[2] / "checkpoints"
...
def resolve_pretrained_reference(path_value: str | os.PathLike[str] | None) -> Path | None:
    if path_value is None:
        return None
    path = Path(path_value).expanduser()
    if path.is_absolute():
        return path
    return resolve_pretrained_path(*path.parts)
